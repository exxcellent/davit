...
export const ConstraintsHelper = {
  deleteDataConstraintCheck(dataId: number, dataStore: DataStoreCTO) {
    ...
    const decisionExists: boolean = Array.from(dataStore.decisions.values()).some(
        (decision) => decision.dataAndInstaceIds.some((dataAndInstaceId) => dataAndInstaceId.dataFk === dataId),
    );

    const chainDecisionExists: boolean = Array.from(dataStore.chaindecisions.values()).some(
        (chainDecision) => chainDecision.dataAndInstanceIds.some((dataAndInstance) => dataAndInstance.dataFk === dataId),
    );
  ...
  deleteStepConstraintCheck(stepId: number, dataStore: DataStoreCTO) {
    ...
    const decisionExists: boolean = Array.from(dataStore.decisions.values()).some(
        (decision) => (decision.ifGoTo.type === GoToTypes.STEP && decision.ifGoTo.id === stepId) || (decision.elseGoTo.type === GoToTypes.STEP && decision.elseGoTo.id === stepId),
    );
